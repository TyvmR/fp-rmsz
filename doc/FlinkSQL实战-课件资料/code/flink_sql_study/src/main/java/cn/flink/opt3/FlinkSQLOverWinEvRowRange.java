...
public class FlinkSQLOverWinEvRowRange {
    ...
    public static void main(String[] args) {
        ...
        Table resultTable = tEnv.sqlQuery("select product_id,max(price) OVER w AS max_price, avg(price) OVER w AS avg_price   from "
                + table + " WINDOW w AS ( PARTITION BY product_id ORDER BY date_time ROWS BETWEEN  3 PRECEDING AND CURRENT ROW) ");
